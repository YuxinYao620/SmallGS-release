...
class Runner:
    ...
    # def fit_depth(self,depth,intrinsic,camera,mask):
    ...
    def train_gs_doma(self,sh_degree_to_use,global_cam_poses_est):
        ...
        # train a first frame
        first_frame_data = self.trainset[0]
        height,width = self.trainset[0]['image'].shape[0:2]
        # renders_0, alphas_0, info_0, loss = self.train_block(first_frame_data['camtoworld'].unsqueeze(0).to(self.device), 
        #                                                      first_frame_data['K'].unsqueeze(0).to(self.device), width, height, 
        #                                                      sh_degree_to_use, [first_frame_data['image_id']], 
        #                                                      first_frame_data['image'].unsqueeze(0).to(self.device),
        #                                                      schedulers,camera_scheduler=None,camera_opt = False,
        #                                                      depthmap_gt=first_frame_data['depth_map'].unsqueeze(0).to(self.device),masks=None) 
        ...
        for step in pbar:
            ...
            if cfg.doma:
                ...
                weight_factor = (1- step/eval_max_step)
                doma_mean = (new_means * weight_factor) + self.splats['means'] *(1-weight_factor)
                # render the splats
